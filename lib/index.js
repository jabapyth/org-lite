...
var OrgFile = exports.OrgFile = function (ipath, full, options) {
  this.fname = path.join(ipath, index_name);
  this.ipath = ipath;
  this.chunks = read(ipath, options);
...
OrgFile.prototype.fillOut = function () {
  ...
  for (var i=0; i<chunks.majors.length; i++) {
    ...
    major.children = load(path.join(ipath, major.properties.slug));
  }
};
...
OrgFile.prototype.remove = function (poid, index) {
  this.dirty = true;
  var parent = this.chunks.ids[poid]
    , child = parent.children.splice(index, 1);
  delete this.chunks.ids[child.prototype.id];
};
...
exports.read = function(fname, options){
  return exports.parse(fs.readFileSync(fname, {encoding: 'utf8'}), fname, options);
};
 ...
 */
var starify = exports.starify = function(lines, fname, options) {
  ...
  for (var i=1; i<lines.length; i++) {
    ...
    if (!curchunk.properties.created && options.time) {
      curchunk.properties.created = options.created;
      chunks.dirty = true;
    }
...
var load = exports.load = function (ipath) {
