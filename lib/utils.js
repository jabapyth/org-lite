...
var index_name = module.exports.index_name = 'index.org';
...
function read(fname, options){
  ...
  var chunks = parse(fs.readFileSync(fname).toString('utf8'), fname, options);
  chunks.fname = fname;
  return chunks;
}
...
function getFull(ipath, options) {
  ...
  for (var i=0; i<chunks.majors.length; i++) {
    ...
    subchunk = getFull(path.join(ipath, major.properties.slug), options);
    for (var id in subchunk.ids) {
      chunks.childids[id] = subchunk;
    }
    for (id in subchunk.childids) {
      chunks.childids[id] = subchunk.childids[id];
    }
    delete subchunk.childids;
    major.children = subchunk.children;
    chunks.subchunks.push(subchunk);
  }
  return chunks;
}
...
function store(ipath, children)  {
  ...
  for (var i=0; i<majors.length; i++) {
    ...
    major._children = major.children;
    major.children = [];
  }
  write(fname, children);
  for (i=0; i<majors.length; i++) {
    major = majors[i];
    major.children = major._children;
    delete major._children;
  }
}

function saveDirty(chunks) {
  for (var i=0; i<chunks.subchunks.length; i++) {
    saveDirty(chunks.subchunks[i]);
  }
  for (i=0; i<chunks.majors.length; i++) {
    chunks.majors[i]._children = chunks.majors[i].children;
    chunks.majors[i].children = [];
  }
  if (chunks.dirty) {
    write(chunks.fname, chunks.children);
  }
  chunks.dirty = false;
  for (i=0; i<chunks.majors.length; i++) {
    chunks.majors[i].children = chunks.majors[i]._children;
    delete chunks.majors[i]._children;
  }
}
